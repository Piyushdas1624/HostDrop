...
class AdversarialBackendTester:
    def __init__(self):
        self.test_dir = tempfile.mkdtemp(prefix="turboshare_adv_test_")
        ...
        turboshare.UPLOAD_DIR = self.test_recv
        turboshare.HOST_SHARE = self.test_share
        ...
        turboshare.SERVER_PORT = self.port
        self.base_url = f"http://127.0.0.1:{self.port}"

        self.server = turboshare.ThreadingHTTPServer(("127.0.0.1", self.port), turboshare.TurboShareHandler)
    ...
    def test_zip_streaming(self):
        ...
        try:
            ...
            with open(os.path.join(zip_target_dir, "readme.txt"), "w", encoding="utf-8") as f:
                f.write("TurboShare High Speed Transfer Hub\nTest File")
            ...
            with zipfile.ZipFile(zip_io, "r") as zf:
                ...
                txt_ok = "TurboShare High Speed Transfer Hub" in read_txt
    ...
    def run_all(self):
        self.log("=" * 72)
        self.log("  TURBOSHARE EMPIRICAL ADVERSARIAL BACKEND STRESS TEST SUITE")
        self.log("=" * 72)
